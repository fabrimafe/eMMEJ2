...
categories = ['DNA_TE', 
    'LINE_TE', 'LTR_TE', 'RC_TE'] # , 'Satelite'
cn = ['DNA TE', 
     'LINE TE', 'LTR TE', 'RC TE'] # ,'Satellite'
...
for d,ds in zip(datasets,datasets_pth_prefix): 
    ...
    pth = f'/home/labs/alevy/guyta/guy_master_project/results/Drosophila/{d}/EMmej_results/20221126/{ds}_EMmej_formated_non_repeated_regions/20221201_blockbootstrap_output/EMmej_overall_estimations.tsv'
    df = pd.read_csv(pth, sep='\t')
    df['cat'] = 'Non-repeated regions'
    for cat,name in zip(categories, cn):
        pth = f'/home/labs/alevy/guyta/guy_master_project/results/Drosophila/{d}/EMmej_results/20221126/{ds}_EMmej_formated_{cat}/20221201_blockbootstrap_output/EMmej_overall_estimations.tsv'
    ...
    for mech,mech_name in zip(mechanisms,mechanisms_name):
        ...
        plt.show()
        for cat in cn:
            if cat != 'Non-repeated regions':
                p_val,p,ann = calc_p_val(vec_b=df.loc[(df['cat'] == 'Non-repeated regions'), mech],
                ...
                tmp_ann_df.loc[0, ['mechanism','group1', 'group2']] = mech, 'Non-repeated regions', cat
                # print(tmp_ann_df)
                ann_df = pd.concat([ann_df, tmp_ann_df])
        fig = ax.get_figure()
        fig.savefig(f"{path_to_output}/{ds}_repeat_ann_{mech}_box.svg", bbox_inches='tight')
